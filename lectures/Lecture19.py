...
def run_module(ctx):
    ...
    ctx['tensors']['full_idx'] = full_idx
    ctx['tensors']['miss_idx'] = miss_idx
    ctx['tensors']['missing'] = missing
    ctx['tensors']['n_missing'] = n_missing
    ctx['tensors']['rand_idx'] = rand_idx
    ctx['tensors']['rng'] = rng
    ctx['tensors']['x'] = x
    ctx['tensors']['x_full'] = x_full
    ctx['tensors']['y'] = y
    ctx['tensors']['y_full'] = y_full
run_module(ctx)
...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    impute = ctx.get('tensors', {}).get('impute')
    missing = ctx.get('tensors', {}).get('missing')
    x_full = ctx.get('tensors', {}).get('x_full')
    y_full = ctx.get('tensors', {}).get('y_full')
...
def run_module(ctx):
    ...
    print(f'R2 on full dataset:  {rf.score(x_full, y_full):.3f}')
    ctx['tensors']['imputed'] = imputed
    ctx['tensors']['missing_cols'] = missing_cols
...
def run_module(ctx):
    ...
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    missing = ctx.get('tensors', {}).get('missing')
    synthetic_data = ctx.get('tensors', {}).get('synthetic_data')
    x_full = ctx.get('tensors', {}).get('x_full')
    y_full = ctx.get('tensors', {}).get('y_full')
    ...
    print(f'R2 on full dataset:  {rf.score(x_full, y_full):.3f}')
    ctx['tensors']['augmented_data'] = augmented_data
    ctx['tensors']['x'] = x
    ctx['tensors']['y'] = y
run_module(ctx)
...
def run_module(ctx):
    ...
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    data = ctx.get('data')
    itertools = ctx.get('tensors', {}).get('itertools')
    import numpy as np
...
def run_module(ctx):
    ...
    ax.legend()
    ctx['tensors']['model_skl'] = model_skl
    ctx['tensors']['not_nan'] = not_nan
    ctx['tensors']['residuals'] = residuals
    ctx['tensors']['rmse'] = rmse
    ctx['tensors']['x_arr'] = x_arr
    ctx['tensors']['x_full'] = x_full
    ctx['tensors']['x_sc'] = x_sc
    ctx['tensors']['x_scaler'] = x_scaler
    ctx['tensors']['xf_sc'] = xf_sc
    ctx['tensors']['y_arr'] = y_arr
    ctx['tensors']['y_full'] = y_full
    ctx['tensors']['y_hat'] = y_hat
    ctx['tensors']['y_sc'] = y_sc
    ctx['tensors']['y_scaler'] = y_scaler
    ctx['tensors']['yf_sc'] = yf_sc
run_module(ctx)
...
def run_module(ctx):
    ...
    ax.set_ylabel('Observed')
    ax.legend()
    ctx['tensors']['colab_path'] = colab_path
    ctx['tensors']['dl_test'] = dl_test
    ctx['tensors']['dl_train'] = dl_train
    ctx['tensors']['ds_test'] = ds_test
    ctx['tensors']['ds_train'] = ds_train
    ctx['tensors']['log_path'] = log_path
    ctx['tensors']['logger'] = logger
    ctx['tensors']['losses'] = losses
    ctx['tensors']['metrics'] = metrics
    ctx['tensors']['model_ft'] = model_ft
    ctx['tensors']['out'] = out
    ctx['tensors']['residuals'] = residuals
    ctx['tensors']['this_loss'] = this_loss
