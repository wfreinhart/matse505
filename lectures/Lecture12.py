...
def run_module(ctx):
    ...
    nn = ctx.get('tensors', {}).get('nn')
    tqdm = ctx.get('tensors', {}).get('tqdm')
    train_and_plot_mlp = ctx.get('tensors', {}).get('train_and_plot_mlp')
    xl = ctx.get('tensors', {}).get('xl')
    xls = ctx.get('tensors', {}).get('xls')
    yls = ctx.get('tensors', {}).get('yls')
    ...
    train_and_plot_mlp(model)
    ctx['model'] = model
    ctx['tensors']['activation'] = activation
    ctx['tensors']['hidden_size'] = hidden_size
    ctx['tensors']['input_size'] = input_size
    ctx['tensors']['output_size'] = output_size
run_module(ctx)
...
def run_module(ctx):
    ...
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    i = ctx.get('tensors', {}).get('i')
    xlt = ctx.get('tensors', {}).get('xlt')
    ylt = ctx.get('tensors', {}).get('ylt')
    ...
    ctx['tensors']['train_data'] = train_data
run_module(ctx)
...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    epoch = ctx.get('tensors', {}).get('epoch')
    loss = ctx.get('tensors', {}).get('loss')
    model = ctx.get('model')
    optimizer = ctx.get('tensors', {}).get('optimizer')
    ...
    ctx['tensors']['checkpoint'] = checkpoint
    ctx['tensors']['epoch'] = epoch
    ctx['tensors']['loss'] = loss
run_module(ctx)
...
def run_module(ctx):
    ...
    model.eval()
    ctx['model'] = model
    ctx['tensors']['traced_model'] = traced_model
run_module(ctx)
...
def run_module(ctx):
    ...
    for l in lines:
        print(l)
    ctx['tensors']['input_tensor'] = input_tensor
    ctx['tensors']['lines'] = lines
run_module(ctx)
...
def run_module(ctx):
    ...
    i = ctx.get('tensors', {}).get('i')
    pl = ctx.get('tensors', {}).get('pl')
    pl_model = ctx.get('tensors', {}).get('pl_model')
    xlt = ctx.get('tensors', {}).get('xlt')
    ylt = ctx.get('tensors', {}).get('ylt')
...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    dl = ctx.get('tensors', {}).get('dl')
    nn = ctx.get('tensors', {}).get('nn')
    pl = ctx.get('tensors', {}).get('pl')
    train_data = ctx.get('tensors', {}).get('train_data')
    class LitMLPRegressor(pl.LightningModule):
        def __init__(self, output_size, hidden_size=(100, ), activation=nn.ReLU()):
            super(LitMLPRegressor, self).__init__()
