...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    github_url = ctx.get('tensors', {}).get('github_url')
    local_path = ctx.get('tensors', {}).get('local_path')
    import pandas as pd
...
def run_module(ctx):
    ...
    print(xs)
    print(ys)
    ctx['tensors']['light_blue'] = light_blue
run_module(ctx)
...
def run_module(ctx):
    ...
    ctx['tensors']['idx'] = idx
    ctx['tensors']['xlist'] = xlist
    ctx['tensors']['ylist'] = ylist
run_module(ctx)
...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    ax2 = ctx.get('tensors', {}).get('ax2')
    data = ctx.get('data')
    fig2 = ctx.get('tensors', {}).get('fig2')
    interpolate = ctx.get('tensors', {}).get('interpolate')
    x = ctx.get('tensors', {}).get('x')
    xs = ctx.get('tensors', {}).get('xs')
    y = ctx.get('tensors', {}).get('y')
    ...
    ax2.legend()
    fig2
    ctx['tensors']['spline'] = spline
    ctx['tensors']['x_hat'] = x_hat
    ctx['tensors']['y_hat'] = y_hat
    ctx['tensors']['zero'] = zero
run_module(ctx)
...
def run_module(ctx):
    ...
    ctx['tensors']['gpr'] = gpr
    ctx['tensors']['kernel'] = kernel
    ctx['tensors']['rng'] = rng
    ctx['tensors']['training_indices'] = training_indices
    ctx['tensors']['x_extrap'] = x_extrap
run_module(ctx)
...
def run_module(ctx):
    ...
    ctx['tensors']['x'] = x
    ctx['tensors']['x3_spline'] = x3_spline
    ctx['tensors']['x_hat'] = x_hat
    ctx['tensors']['x_test'] = x_test
    ctx['tensors']['y'] = y
    ctx['tensors']['y3_spline'] = y3_spline
    ctx['tensors']['y_hat'] = y_hat
run_module(ctx)
