...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    pyplot = ctx.get('tensors', {}).get('pyplot')
...
def run_module(ctx):
    ...
    ctx['tensors']['my_data'] = my_data
    ctx['tensors']['my_matrix'] = my_matrix
run_module(ctx)
...
def run_module(ctx):
    ...
    print(kurt_fisher)
    ctx['tensors']['kurt'] = kurt
    ctx['tensors']['kurt_fisher'] = kurt_fisher
    ctx['tensors']['sigma'] = sigma
    ctx['tensors']['skew'] = skew
run_module(ctx)
...
def run_module(ctx):
    ...
    print(r)
    ctx['data'] = xyz_data
    ctx['tensors']['data_drop'] = data_drop
    ctx['tensors']['data_filtered'] = data_filtered
...
def run_module(ctx):
    ...
    ctx['tensors']['y_clean'] = y_clean
    ctx['tensors']['z_clean'] = z_clean
    ctx['tensors']['z_model'] = z_model
run_module(ctx)
...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    result = ctx.get('tensors', {}).get('result')
    z = ctx.get('tensors', {}).get('z')
    z_model = ctx.get('tensors', {}).get('z_model')
    residual = z - z_model
