...
def run_module(ctx):
    ...
    ctx['tensors']['permuted'] = permuted
    ctx['tensors']['result'] = result
    ctx['tensors']['x_permuted'] = x_permuted
run_module(ctx)
...
def run_module(ctx):
    ...
    calc_rmse = ctx.get('tensors', {}).get('calc_rmse')
    it = ctx.get('tensors', {}).get('it')
    permuted = ctx.get('tensors', {}).get('permuted')
    x = ctx.get('tensors', {}).get('x')
    ...
    ctx['tensors']['baseline'] = baseline
    ctx['tensors']['pca'] = pca
    ctx['tensors']['permuted_sc'] = permuted_sc
    ctx['tensors']['x_permuted'] = x_permuted
    ctx['tensors']['x_sc'] = x_sc
    ctx['tensors']['z'] = z
run_module(ctx)
...
def run_module(ctx):
    ...
    ctx['tensors']['baseline'] = baseline
    ctx['tensors']['pca'] = pca
    ctx['tensors']['permuted_aug'] = permuted_aug
    ctx['tensors']['x_aug'] = x_aug
    ctx['tensors']['x_permuted'] = x_permuted
    ctx['tensors']['z'] = z
run_module(ctx)
...
def run_module(ctx):
    ...
    ctx['tensors']['x'] = x
    ctx['tensors']['x_dropped'] = x_dropped
    ctx['tensors']['y'] = y
run_module(ctx)
...
def run_module(ctx):
    ...
    ax.legend()
    ctx['model'] = model
    ctx['tensors']['base_test'] = base_test
    ctx['tensors']['base_train'] = base_train
    ctx['tensors']['permuted'] = permuted
    ctx['tensors']['x_permuted'] = x_permuted
run_module(ctx)
...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    baseline_rf = ctx.get('tensors', {}).get('baseline_rf')
    it = ctx.get('tensors', {}).get('it')
    permutation_importance = ctx.get('tensors', {}).get('permutation_importance')
    permuted_rf = ctx.get('tensors', {}).get('permuted_rf')
    x = ctx.get('tensors', {}).get('x')
    xtest = ctx.get('tensors', {}).get('xtest')
    xtrain = ctx.get('tensors', {}).get('xtrain')
    ytest = ctx.get('tensors', {}).get('ytest')
    ...
    ax.legend()
    ctx['model'] = model
    ctx['tensors']['delta_percent_lin'] = delta_percent_lin
    ctx['tensors']['delta_percent_rf'] = delta_percent_rf
run_module(ctx)
...
def run_module(ctx):
    ...
    ctx['model'] = model
    ctx['tensors']['baseline'] = baseline
    ctx['tensors']['constructors'] = constructors
    ctx['tensors']['delta_percent'] = delta_percent
    ctx['tensors']['names'] = names
...
def run_module(ctx):
    ...
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    calc_rmse = ctx.get('tensors', {}).get('calc_rmse')
    constructors = ctx.get('tensors', {}).get('constructors')
...
def run_module(ctx):
    ...
    data_cl
    ctx['tensors']['data_cl'] = data_cl
    ctx['tensors']['filename_cl'] = filename_cl
    ctx['tensors']['github_url_cl'] = github_url_cl
    ctx['tensors']['local_path_cl'] = local_path_cl
run_module(ctx)
...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    data_cl = ctx.get('tensors', {}).get('data_cl')
    drop_column_importance = ctx.get('tensors', {}).get('drop_column_importance')
    it = ctx.get('tensors', {}).get('it')
    neural_network = ctx.get('tensors', {}).get('neural_network')
    permutation_importance = ctx.get('tensors', {}).get('permutation_importance')
    ...
    ctx['model'] = model
    ctx['tensors']['baseline'] = baseline
    ctx['tensors']['constructors_cl'] = constructors_cl
    ctx['tensors']['delta'] = delta
    ctx['tensors']['names'] = names
    ctx['tensors']['results'] = results
    ctx['tensors']['short_model_name'] = short_model_name
    ctx['tensors']['x_cl'] = x_cl
    ctx['tensors']['xticks'] = xticks
    ctx['tensors']['y_cl'] = y_cl
run_module(ctx)
