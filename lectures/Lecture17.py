...
def run_module(ctx):
    ...
    rgb2grey = ctx.get('tensors', {}).get('rgb2grey')
    ...
    ctx['tensors']['dl_valid'] = dl_valid
    ctx['tensors']['ds_train'] = ds_train
    ctx['tensors']['ds_valid'] = ds_valid
...
def run_module(ctx):
    ...
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    dl_train = ctx.get('tensors', {}).get('dl_train')
    dl_valid = ctx.get('tensors', {}).get('dl_valid')
    model = ctx.get('model')
    ...
    for i, alpha in enumerate(np.linspace(0, 1, n_interp)):
        ...
        axes[i].imshow(arr.detach().squeeze(0).numpy(), 'Greys_r')
    ctx['tensors']['arr'] = arr
    ctx['tensors']['batch_loss'] = batch_loss
    ctx['tensors']['loss'] = loss
    ctx['tensors']['n_interp'] = n_interp
...
def run_module(ctx):
    ...
    for id in center_id:
        ...
        ab = AnnotationBbox(image_offset, coefs[id, :2], xycoords='data', frameon=False)
        ax.add_artist(ab)
    ctx['tensors']['all_y'] = all_y
    ctx['tensors']['all_z'] = all_z
    ctx['tensors']['center_id'] = center_id
    ctx['tensors']['coefs'] = coefs
    ctx['tensors']['dist'] = dist
    ctx['tensors']['image_offset'] = image_offset
    ctx['tensors']['img'] = img
    ctx['tensors']['img_tensor'] = img_tensor
    ctx['tensors']['pca'] = pca
