...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    mlp_fitness = ctx.get('tensors', {}).get('mlp_fitness')
    neural_network = ctx.get('tensors', {}).get('neural_network')
    ...
    trial = ctx.get('tensors', {}).get('trial')
    ...
    ctx['model'] = model
    ctx['tensors']['best_objectives'] = best_objectives
    ctx['tensors']['results'] = results
run_module(ctx)
...
def run_module(ctx):
    ...
    import sklearn
    from sklearn import metrics, model_selection, linear_model, cluster, decomposition, preprocessing, ensemble, tree, neighbors, svm, mixture
    choose_worst_feature = ctx.get('tensors', {}).get('choose_worst_feature')
    drop_column_importance = ctx.get('tensors', {}).get('drop_column_importance')
    fitness = ctx.get('tensors', {}).get('fitness')
    pygad = ctx.get('tensors', {}).get('pygad')
    x = ctx.get('tensors', {}).get('x')
    y = ctx.get('tensors', {}).get('y')
    # !pip install pygad
    ...
    ctx['model'] = model
    ctx['tensors']['all_features'] = all_features
    ctx['tensors']['crossover_type'] = crossover_type
    ctx['tensors']['feature_order'] = feature_order
    ctx['tensors']['features'] = features
    ctx['tensors']['fitness_function'] = fitness_function
    ctx['tensors']['ga_instance'] = ga_instance
    ctx['tensors']['keep_parents'] = keep_parents
    ctx['tensors']['last_feature'] = last_feature
    ctx['tensors']['mutation_percent_genes'] = mutation_percent_genes
    ctx['tensors']['mutation_type'] = mutation_type
    ctx['tensors']['num_generations'] = num_generations
    ctx['tensors']['num_genes'] = num_genes
    ctx['tensors']['num_parents_mating'] = num_parents_mating
    ctx['tensors']['parent_selection_type'] = parent_selection_type
    ctx['tensors']['sol_per_pop'] = sol_per_pop
    ctx['tensors']['solutions'] = solutions
    ctx['tensors']['top_features_rfe'] = top_features_rfe
    ctx['tensors']['x_train_trim'] = x_train_trim
    ctx['tensors']['x_val_trim'] = x_val_trim
run_module(ctx)
